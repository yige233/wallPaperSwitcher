...
`-j`参数可以为固定到任务栏的快捷方式添加跳转列表，就像右键Edge浏览器图标时会看到的“新建窗口”“新建无痕窗口”一样。先在终端中执行`WallpaperApp -j`，看到`JumpList 更新成功`后，将`WallpaperApp.exe`拖放到任务栏。右键这个图标，就能看到刚刚添加的菜单了。左键单击图标（也就是直接执行）相当于“启动/切换至下一张壁纸”。
